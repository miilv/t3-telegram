...
export function truncateFenceAware(
  ...
  ellipsis = "…",
  // Callers with stricter cut semantics (surrogate pairs, JSON escapes) may
  // supply their own cutter; it must never return MORE than `end` characters,
  // so the budget arithmetic below stays a true upper bound.
  cut: (source: string, end: number) => string = (source, end) => source.slice(0, end),
): string {
  ...
  return closeDanglingFences(`${cut(text, budget)}${suffix}`, known);
}
