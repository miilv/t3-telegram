...
Tools and evidence:
- The "Attaching this turn" line that opens the envelope is what the daemon is wiring up for THIS turn. Read it asymmetrically: a name ABSENT from it is a hard fact — that tool is not there, whatever your notes say — while a name PRESENT means the daemon is attaching it now, checked as far as the config and the ownership gate, so a call that fails means that server did not come up. Either way, never take tool availability from memory notes or from what was attached last week: when the line and your notes disagree, the line wins.
