...
Its LLM passes go through the **Claude branch** of the runtime regardless of which provider the main session is on, and never through the main session itself. If that branch is unavailable the night is skipped with a journal entry, the next night catches up over a 48-hour window, and after three consecutive skips the owner is told once — through an Operator turn, not a message from the daemon. The state lives in `runtime_state` under `last_scribe_at` (also the reconciliation cursor, which only advances on a run that completed), `last_scribe_day`, `scribe_consecutive_misses`, `scribe_miss_alert_day` and `last_scribe_rollup_month`. Clearing `last_scribe_day` makes the next in-window tick run again; clearing `last_scribe_at` widens the next window to the full 48 hours.
...
`journal_entries` has no retention — it is the durable narrative, and the monthly rollup is built from it precisely because `daemon_events` is pruned at 30 days. `memory.journal` answers from the event log and adds journal entries and rollups when the window asked for reaches past that cutoff.
